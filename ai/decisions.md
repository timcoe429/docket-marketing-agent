...
The droplet agent reports status, logs, and blog drafts to **Base44** via REST (`Agent`, `AgentLog`, `BlogPost`, `SiteAudit`, `ContentAction`, `PlannedPost`, `CROSnapshot`, `CRORecommendation`, `CROKnowledgeBase` entities). **Base44 is the single source of truth** for generated draft content and the monthly content plan / action queue (no Google Sheets/Drive/Docs in this flow). The agent exposes **Express** APIs: `GET /health`, `POST /run` (weekly content pipeline), `POST /run/audit` (fire-and-forget monthly audit for all brands), `POST /run/cro` (fire-and-forget CRO agent for Docket), `POST /run/cro-knowledge` (fire-and-forget monthly CRO knowledge refresh for Docket), `POST /run/publish` (body: `{ blogPostId, brand }` — pushes a **WordPress draft** after approval or manual trigger), `GET /status`. The Next.js **dashboard** remains the Supabase-backed UI for jobs and is separate from Base44.
...
`agent/skills/content-pipeline.js` implements the **weekly content pipeline** (sitemap, GSC striking distance, GA4, Claude, Base44 `BlogPost`, publish helper). `agent/skills/site-audit.js` implements the **monthly site audit** (full GSC 1–100, top 100 GA4 pages, Claude audit JSON, Base44 `SiteAudit` / `ContentAction` / `PlannedPost`). `agent/skills/cro-agent.js` implements the **CRO agent** (Docket money pages: GA4 `generate_lead` + sessions by device for two 14-day windows, Puppeteer screenshots, Claude with web search, Base44 `CROSnapshot` per page + one `CRORecommendation` per run). `agent/skills/cro-knowledge-base.js` implements the **CRO knowledge base** monthly refresh (Claude + web search → Base44 `CROKnowledgeBase`). Other skill files under `agent/skills/` and `agent/lib/ssh.js`, when added, remain placeholders until those skills are implemented — do not remove them arbitrarily.
...
- **Base44 `CRORecommendation`:** `brand`, `page_path`, `page_name`, `observation`, `benchmark_comparison`, `benchmark_gap`, `hypothesis`, `what_to_test`, `device_focus`, `priority`, `reasoning`, `status` (`pending` / `testing` / `completed` / `skipped`), `created_at` (YYYY-MM-DD ET). Use [`updateCRORecommendation`](../agent/lib/base44.js) for status updates from workflows outside the agent.

## CRO knowledge base (Base44)
- **`CROKnowledgeBase`** stores SaaS-wide CRO research as **JSON in `content`** (stringified full object from Claude). **`benchmarks`** duplicates the benchmarks subtree as stringified JSON for quick access. Also **`brand`**, **`last_updated`**, **`summary`** (one-line description). Align the entity in Base44 with [`agent/lib/base44.js`](../agent/lib/base44.js) `getCROKnowledgeBase` / `upsertCROKnowledgeBase`.
- **Monthly cron** 1st of month 7:00 AM `America/New_York` (`0 7 1 * *`) and manual **`POST /run/cro-knowledge`** run [`updateCROKnowledgeBase`](../agent/skills/cro-knowledge-base.js) for Docket. Knowledge base update failures are logged to AgentLog and do not crash Express.
- **CRO agent** reads **`CROKnowledgeBase`** and **`CRORecommendation`** rows with `status=testing` **before each bi-weekly analysis** (immediately before the per-page Claude loop). If `getCROKnowledgeBase` returns null, analysis continues with placeholder context (no crash).
